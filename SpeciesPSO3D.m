function [arcx, arcy, arcAlt, arcz, kBestInterp, tBestInterp, omegaBestInterp, deltaSBestInterp, BestCosts, swarmVecPre, swarmVecCurr, swarmCost] = SpeciesPSO3D (gamma1, gamma2, swarmIndAvoiding, posIniIndiv, posIniSwarmRel, posIniSwarmRelNext, x0, y0, Alt0, z0, zForbid, k, t, ome, dS, arcxPre, arcyPre, arcAltPre, arczPre, smoothField, EextField, Maxiter, N) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 
% k, ome, dS: three dimensions of PSO, curvature, slope angle and arc
...
searchBound2=size(ome, 2); 
searchBound3=size(dS, 1); 
searchBound4=size(t, 1); 
searchBound=[searchBound1, searchBound2, searchBound3, searchBound4]; 
%==========================================================================
% Parameters of PSO
%==========================================================================
PopulationSize = 200; % Enter Population Size (Swarm size)
N_variables = 4; % Enter number of Design Variables 
...
%     dParticle = sum(sum(sqrt(dParticleX.^2 + dParticleY.^2 + dParticleZ.^2)))./(PopulationSize-1); 
    ...
    Particle1Left = Particle_Position(:,1,:);
    Particle2Left = Particle_Position(:,2,:); 
    Particle3Left = Particle_Position(:,3,:); 
    Particle4Left = Particle_Position(:,4,:); 
    Particle1Right = permute(Particle_Position(:,1,:), [2 1 3]); 
    Particle2Right = permute(Particle_Position(:,2,:), [2 1 3]); 
    Particle3Right = permute(Particle_Position(:,3,:), [2 1 3]); 
    Particle4Right = permute(Particle_Position(:,4,:), [2 1 3]); 
    dParticle = sum(sqrt((Particle1Left(:, ones(1, PopulationSize), :)-Particle1Right(ones(1, PopulationSize), :, :)).^2 + ...
        (Particle2Left(:, ones(1, PopulationSize), :)-Particle2Right(ones(1, PopulationSize), :, :)).^2 + ...
        (Particle3Left(:, ones(1, PopulationSize), :)-Particle3Right(ones(1, PopulationSize), :, :)).^2 + ... 
        (Particle4Left(:, ones(1, PopulationSize), :)-Particle4Right(ones(1, PopulationSize), :, :)).^2), 2)./(PopulationSize); 
    
    kInterp=zeros(1,PopulationSize,speciesNum); 
    tInterp=zeros(1,PopulationSize,speciesNum); 
    ...
    for ind_i=1: speciesNum
        ...
        tInterp(:,:,ind_i)=interp1(t(:,:,ind_i),Particle_Position(:,4,ind_i)); 
        omegaInterp(:,:,ind_i)=interp1(ome(:,:,ind_i),Particle_Position(:,1,ind_i));
        deltaSInterp(:,:,ind_i)=interp1(dS(:,:,ind_i),Particle_Position(:,3,ind_i));
...
%     [arcxTemp, arcyTemp, x1, y1]=arcParameterization (x0, y0, kInterp, omegaInterp, deltaSInterp, N); 
    ...
    [arcxTemp, arcyTemp, arcAltTemp, x1, y1]=arcParameterization3D (x0, y0, Alt0, kInterp, tInterp, omegaInterp, deltaSInterp, N); 
...
%     [E, Eint, Eext, Eswm, vecPre, vecCurr] = snakeCostFun (gamma1, gamma2, swarmIndAvoiding, posIniSwarmRel, posIniSwarmRelNext, posIniIndiv, arcxTemp, arcyTemp, arczTemp, zForbid, arcxPre, arcyPre, arczPre, EextField, smoothField);
    ...
    [E, Eint, Eext, Eswm, vecPre, vecCurr] = snakeCostFun3D (gamma1, gamma2, swarmIndAvoiding, ...
        posIniSwarmRel, posIniSwarmRelNext, posIniIndiv, arcxTemp, arcyTemp, arcAltTemp, arczTemp, ...
        zForbid, arcxPre, arcyPre, arcAltPre, arczPre, EextField, smoothField);
...
%         swarmVecCurr(:,:,ind_i)=vecCurr(:,I(ind_i),ind_i); 
    ...
    tBestInterp=zeros(1,1,speciesNum);
    ...
    for ind_i=1: speciesNum
        ...
        tBestInterp(:,:,ind_i)=interp1(t(:,:,ind_i),GlobalBestPlt(:,4,ind_i));
        omegaBestInterp(:,:,ind_i)=interp1(ome(:,:,ind_i),GlobalBestPlt(:,1,ind_i));
        deltaSBestInterp(:,:,ind_i)=interp1(dS(:,:,ind_i),GlobalBestPlt(:,3,ind_i));
...
%     [arcx, arcy, x1, y1]=arcParameterization (x0, y0, kBestInterp, omegaBestInterp, deltaSBestInterp, N); 
    ...
    [arcx, arcy, arcAlt, x1, y1]=arcParameterization3D (x0, y0, Alt0, kBestInterp, tBestInterp, omegaBestInterp, deltaSBestInterp, N);

    arcz = interp2(smoothField, arcx, arcy); 
end 

kNew=zeros(1,1,speciesNum); 
tNew=zeros(1,1,speciesNum); 
omegaNew=zeros(1,1,speciesNum); 
deltaSNew=zeros(1,1,speciesNum); 
for ind_i=1: speciesNum 
    kNew(:,:,ind_i)=interp1(k(:,:,ind_i),GlobalBestPlt(:,2,ind_i)); 
    tNew(:,:,ind_i)=interp1(t(:,:,ind_i),GlobalBestPlt(:,4,ind_i)); 
    omegaNew(:,:,ind_i)=interp1(ome(:,:,ind_i),GlobalBestPlt(:,1,ind_i)); 
    deltaSNew(:,:,ind_i)=interp1(dS(:,:,ind_i),GlobalBestPlt(:,3,ind_i)); 
end 