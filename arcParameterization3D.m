function [x, y, z, x1, y1]=arcParameterization3D (x0, y0, z0, k, t, omega, deltaS, N) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% clear;
% clc;
% swarmSize=2; 
% theta=linspace(-pi,pi,swarmSize+1);
% x0=95; 
% y0=95; 
% r=20; 
% xi=r*cos(theta)+x0;
...
% posIniIndivAlt=zeros(1, swarmSize)+200; 
% x0 = reshape(posIniIndiv(1,:),1,1,swarmSize); 
% y0 = reshape(posIniIndiv(2,:),1,1,swarmSize); 
% z0 = reshape(posIniIndivAlt,1,1,swarmSize); 
% k = zeros(1, 1, swarmSize)+.1; 
% t = zeros(1, 1, swarmSize)-5; 
% omega = zeros(1, 1, swarmSize);
% deltaS=5*5; 
% N=100; 
...
z=zeros(N, PopulationSize* swarmSize); 

linSpcInd = linspace(0,1,N); 
zTempPos=ones(N,1)*z0(:)'+linSpcInd'*ones(1,length(t(:))).*(ones(N,1)*(z1(:)-z0(:))'); 
zTempNeg=ones(N,1)*z0(:)'-linSpcInd'*ones(1,length(t(:))).*(ones(N,1)*(z0(:)-z1(:))'); 
z(:, t==0)=z(:, t==0)+z0(:, t==0); 
z(:, t>0)=zTempPos(:, t>0); 
z(:, t<0)=zTempNeg(:, t<0); 
...
z=reshape(z, [N, PopulationSize, swarmSize]); 
